...
\item Compute the genesis of cyber protocol based on the deined distribution
...
The \linkred{https://github.com/cybercongress/go-cyber}{go-cyber} implementation of cyberlinks is available in the \linkred{https://github.com/cybercongress/dot-cyber}{.cyber} app of experimental web3 browser - \linkred{https://cyb.ai}{cyb}, or at \linkred{http://cyber.page}{cyber.page}.
...
Prior to Genesis cyber\~{}Foundation has minted 750 000 000 000 000 THC (seven hundred fifty terathc), which will be broken down as follows:
...
A game for Cosmos hodlers in ATOM. Participants donate ATOM in exchange for CYB. The more ATOM dontaed, higher the price of CYB. The game is over when either 146 days have passed since the beginning of the Takeoff donations or if the price have increased 5x from the starting price. The price of {\hyperref[cyb]{CYB}} during Takeoff is defined by the following function:

\begin{algorithm}
\caption{CYB price}\label{takeoff-price}
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\Input{Amount of tokens sold $S$;}
\Output{40 * S + 1000}
\end{algorithm}

The key idea is, the better the Takeoff donation round performs, the more payouts the participants in the disciplines will receive. 100 {\hyperref[cyb]{TCYB}} is allocated to the participant of the Takeoff donations and 50 {\hyperref[cyb]{TCYB}} is allocated for participants of the Game of Links disciplines. All {\hyperref[cyb]{CYB}} tokens that remain after Takeoff are allocated to community pool at the end of the game. All {\hyperref[cyb]{CYB}} tokens that remain after disciplines are allocated to cyber\~{}Congress. In addition to CYB Game of Link token EUL allocated to all Takeoff donors. A \linkred{https://cybercongress.ai/game-of-links/}{detailed document} has been published with rules and provisions for the game.

\titleSection{Cyber\~{}Auction}\label{cyber-auction}

A game for Ethereum hodlers in ETH. Participants donate ETH in exchange for THC. More ETH donated, higher THC price. The game is over when either 888 days have passed or if the price have increased 5x from the starting price. During this phase, {\hyperref[cyb]{CYB}} tokens are continuously distributed by cyber\~{}Congress, based on the vested {\hyperref[thc]{THC}} tokens until the end of the auction. Vested {\hyperref[thc]{THC}} tokens give the ability to receive {\hyperref[cyb]{CYB}} tokens accordingly, and, voting powers within cyber\~{}Foundation. The price of {\hyperref[thc]{THC}} during Cyber\~{}Auction is defined by the following function:

\begin{algorithm}
\caption{THC price}\label{auction-price}
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\Input{Closing takeoff price of CYB in ETH $P$; \\ Amount of tokens sold $S$;}
\Output{1/3 * S * P + 5 * S}
\end{algorithm}

The starting price designed to give takeoff participants 5x premium for risk of investing into hardware and software infrastructure prior to Genesis. cyber\~{}Auction give significant incentives for early participators. After the end of the distribution, participants will be able to unlock their {\hyperref[thc]{THC}} tokens and use them as they wish, e.i. transfer, trade, etc. As a result of the auction, the community will have access to all the donated ETH within the Aragon organization. After the end of cyber\~{}Auction, all the remaining {\hyperref[thc]{THC}} in the cyber\~{}Auction contract must be provably burned. The following rules apply to the {\hyperref[cyb]{CYB}} tokens under the \linkgreen{https://cyber.page/network/cyber/contract/cyber147drnke9676972jr3anklkj7pzgwjw47cp2u7j}{multisig for distribution}:
