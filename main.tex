...
Using merkle trees we can effectively proof the rank for any given content address. While relevance is still subjective by nature, we have a collective proof that something was relevant to a certain community at some point in time.
...
\item payment buttons can be embedded right into search snippets. This means that an agent can interact with the search results, e.g. can buy an item right in .cyber. This means that e-commerce can flourish fairly thanks to a provable conversion attribution
...
uses: (1) staking for consensus, (2) bandwidth limiting for submitting cyberlinks, and (3) expression of will for the computing
of cyber•rank
\end{itemize}
